...
import numpy as np
import pandas as pd
from keras_facenet import FaceNet  # This import may not be needed if not using FaceNet anymore
...
from torchvision import transforms  # Added for image transformations
from iresnet import IResNet, IBasicBlock  # Make sure these are correctly defined or imported
...
# Assuming IResNet model definition is here
# Configuration
...
output_path = "lustre/rsc_mgt_wi-st-sccs-lj7uaansp4q/users/yassin.terraf/multimodal_speaker_recognition/features/CLEANSING/Gint/Ravdess/Noise"
...
# Initialize MTCNN with a threshold of 0.98
detector = MTCNN()
...
def extract_visual_embedding_with_resnet50(image_path, save_noisy_image=True):
    original_image = cv2.imread(image_path)  # Load the actual image from the path using OpenCV
    ...
    if faces:
        ...
        final_face_image_pil = Image.fromarray(final_face_image)
        ...
        face_tensor = preprocess(final_face_image_pil).unsqueeze(0).to('cuda' if torch.cuda.is_available() else 'cpu')
...
def process_file(audio_path, image_path, speaker_label):
    ...
    if os.path.exists(audio_path) and os.path.exists(image_path):
        ...
        visual_embedding = extract_visual_embedding_with_resnet50(image_path)
