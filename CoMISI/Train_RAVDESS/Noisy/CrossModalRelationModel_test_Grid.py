...
class EnhancedCMIFModule(nn.Module):
    ...
    def forward(self, audio_features, visual_features, return_embeddings=False):
        ...
        relation = self.relation_distill(audio_emphasized * visual_emphasized)
        enhanced_audio = audio_emphasized + relation
        enhanced_visual = visual_emphasized + relation
        if return_embeddings:
            return enhanced_audio, enhanced_visual, audio_hidden, visual_hidden
        return enhanced_audio, enhanced_visual

class MultimodalNetworkCoMISI(nn.Module):
    def __init__(self, hidden_size=128, dropout_rate=0.5):  # Adjusted dropout rate
        ...
        self.audio_dropout = nn.Dropout(dropout_rate)  # Dropout for audio features
        self.visual_dropout = nn.Dropout(dropout_rate)  # Dropout for visual features
        self.cmr_module = EnhancedCMIFModule(192, 512, hidden_size)
        self.fusion_fc = nn.Linear(hidden_size * 2, hidden_size)
        self.fusion_dropout = nn.Dropout(dropout_rate)  # Dropout after fusion
        self.classifier = nn.Linear(hidden_size, 34)

    def forward(self, audio_features, visual_features, return_all_embeddings=False):
        audio_features = self.audio_dropout(audio_features)  # Apply dropout to raw audio features
        visual_features = self.visual_dropout(visual_features)  # Apply dropout to raw visual features
        audio_hidden = self.audio_fc(audio_features)
        visual_hidden = self.visual_fc(visual_features)
        
        if return_all_embeddings:
            # Simple concatenation of audio and visual embeddings
            concatenated = torch.cat((audio_hidden, visual_hidden), dim=1)
        ...
        fused_features = self.fusion_dropout(fused_features)  # Apply dropout after fusion
        output = self.classifier(fused_features)
        
        if return_all_embeddings:
            return output, audio_hidden, visual_hidden, concatenated, enhanced_audio, enhanced_visual
        else:
            return output