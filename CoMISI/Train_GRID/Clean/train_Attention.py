...
def train_and_evaluate(model, train_loader, val_loader, test_loader, patience=5):
    ...
    best_model_path = 'best_model_attention_Grid_clean.pth'  # Update path as necessary
    epochs_no_improve = 0  # Initialize counter for early stopping

    for epoch in range(EPOCHS):
        model.train()
        for batch in train_loader:
            audio_feature, visual_feature, label = batch
            optimizer.zero_grad()
            outputs = model(audio_feature, visual_feature)  # Assuming the model takes both features
...
if __name__ == "__main__":
    ...
    results_file_path = '.../Results/CoMISI/GRID/clean/CrossAttention_Results.txt'  # Adjust the path as necessary
